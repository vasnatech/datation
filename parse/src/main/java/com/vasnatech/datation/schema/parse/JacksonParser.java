...
public class JacksonParser implements Parser {
    ...
    public Schemas parse(InputStream in) throws IOException {
        return parse(jsonFactory.createParser(in));
    }

    public Schemas parse(JsonParser parser) throws IOException {
        parser.nextToken();
        if (parser.currentToken() == JsonToken.START_OBJECT) {
            ...
            while (parser.currentToken() == JsonToken.FIELD_NAME) {
                ...
                if ("meta".equals(fieldName)) {
                    parseMeta(parser, databaseBuilder);
                } else {
                    Schema.Builder schemaBuilder = Schema.builder();
                    schemaBuilder.name(fieldName);
                    parseSchema(parser, schemaBuilder);
                    databaseBuilder.schema(schemaBuilder.build());
                }
                parser.nextToken();
            }
    ...
    private void parseMeta(JsonParser parser, Schemas.Builder databaseBuilder) throws IOException {
        parser.nextToken();
        if (parser.currentToken() == JsonToken.START_OBJECT) {
            parser.nextToken();
            while (parser.currentToken() == JsonToken.FIELD_NAME) {
    ...
    void parseSchema(JsonParser parser, Schema.Builder schemaBuilder) throws IOException {
        parser.nextToken();
        if (parser.currentToken() == JsonToken.START_OBJECT) {
            parser.nextToken();
            while (parser.currentToken() == JsonToken.FIELD_NAME) {
    ...
    private void parseTables(JsonParser parser, Schema.Builder schemaBuilder, boolean definitions) throws IOException {
        parser.nextToken();
        if (parser.currentToken() == JsonToken.START_OBJECT) {
            parser.nextToken();
            while (parser.currentToken() == JsonToken.FIELD_NAME) {
                ...
                if (parser.currentToken() == JsonToken.START_OBJECT) {
                    ...
                    while (parser.currentToken() == JsonToken.FIELD_NAME) {
                        ...
                        if ("column-anchor".equals(fieldName)) {
                            appendBuilder.columnAnchor(Anchor.valueOf(parser.nextTextValue().toUpperCase(Locale.ENGLISH)));
                        }
                        parser.nextToken();
                    }
